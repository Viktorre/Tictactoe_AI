...
public class GameEngine {
    ...
    public String[] player_move_on_board(String[] board, String game_status, String player_1, String player_2)
        ...
        while (move_is_not_finished) {
            ...
            input = return_input_from_player_or_ai(board, game_status,player_1,player_2);//keyboard.readLine();
            if (!Arrays.asList(list_of_correct_user_inputs).contains(input)) {
                continue;
            }
            if (InputProcessor.check_if_player_wants_to_quit(input)) {
                System.out.println("game quit");
                board = GameEngine.create_draw();
                return board;
            }
            if (InputProcessor.check_if_field_choice_is_taken(board, input)) {
        ...
        if (game_status=="Player 1 moves next!") {
            board = InputProcessor.update_board(board, input, "X");
        }
        if (game_status=="Player 2 moves next!") {
            board = InputProcessor.update_board(board, input, "@");
        }
        OutputCreator.print_field(board);
        return board;
    }

    private static String[] create_draw() {
        return new String[]{"O", "O", "O", "O", "O", "O", "O", "O", "O"};
    }

    private String return_input_from_player_or_ai(String[] board, String game_status, String player_1,
        ...
        if (Objects.equals(selected_player, "human")) {
            ...
            return make_smart_ai_move(board);
        }
    }

    public void print_field(String[] board) {
        OutputCreator.print_field(board);
    }
    ...
    public String update_game_status(String[] board, String game_status) {
        //this fct checks if game is over and
      if (InputProcessor.check_if_game_is_won_and_notify_if_so(board)) {
          return "Game Over";
          }
      if (InputProcessor.check_if_game_is_a_draw_and_notify_if_so(board)) {
          return "Game Over";
          }
      if (game_status=="Player 1 moves next!") {
